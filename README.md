...
自己搭了一套 Forgejo 当代码仓库和 CI。代码必须走 PR，过八道检查才能合进主干——编译、
测试、查密钥、查漏洞这些。发布不自动，得人手动点，跑完会给镜像签名、出一份物料清单。
部署前脚本再验一遍签名，验不过就不让上。

下面的每一节，都是把上面这段话里的某一个词拆开讲**它到底怎么落地、以及在这个平台上
实测是什么行为**——因为好几处跟通行说法不一样。
