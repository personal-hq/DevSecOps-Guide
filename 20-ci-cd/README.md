...
## 九、失败演练：三条都要真的红（2026-08-29 实做）

「绿的能过」只证明了一半。下面三条是**故意把它弄红**，确认闸真的会拦。
没做过这三条的门禁，不能算验收通过。

### 演练 1 · 提交一个真密钥 → 合并被拦

往 PR 里放一个语法合法的 AWS 凭据（**不要用 `AKIAIOSFODNN7EXAMPLE`**，那是 AWS 文档
示例、在 gitleaks 内置白名单里，用它演练会得到假绿——见
[00-principles](../00-principles.md) 第 4 条）。

结果：`ci / gitleaks` failure → 合并 **405 Not all required status checks successful**。
**人已经点过 Approve 也照样拦**。

> ⚠️ **附带发现：一个分支上的密钥会让全仓每个 PR 都红。**
> `actions/checkout` 配 `fetch-depth: 0` 会拉到**所有分支**的提交，
> 而 `gitleaks detect --source .` 扫全部可达 commit。
> 实测隔离验证：从干净 main 开的 PR，gitleaks 红；**删掉那个含密钥的分支后，
> 同一个 PR 立刻转绿**。
>
> 好处是泄露无处可藏（符合"删掉文件没用，历史里还在"）；
> 代价是**一次误提交阻塞所有人**，而报错指向当前 PR，看起来像"我这个改动有问题"。
> 处置顺序是：先轮换密钥，再删分支/清历史，不要反过来。

### 演练 2 · 例外过期 → CI 红

把 `.trivyignore` 里一条的到期日改成过去。结果精确到行：

```
例外校验失败：
  .trivyignore:11 CVE-2026-14257 —— 已于 2026-08-01 过期（负责人 @platform-team）
```

**这条能红，靠的是 CI 里那个解析脚本**，不是靠自觉。校验器自己要带反向测试
（去掉一行到期日 → 退出码必须为 1），否则你不知道它在不在判。

### 演练 3 · 扫描器拉不到 → 红，但报错指向错的地方

把 trivy 版本改成不存在的 `v9.99.99`。门禁确实红了（fail-closed 正确），
**但报错是**：
...
完全没提 404 或版本不存在。根因是 **`curl -sSL -o` 不加 `-f` 时 HTTP 404 退出码仍为 0**，
把错误页写进了文件，真正的失败落在下一个命令上。排查的人会去查 tar，不会去查 URL。

**所有下载扫描器的 curl 都要加 `-f`。**

### 标签保护是按模式的

同一个仓，`v*` 受保护、白名单只有 `forgejo-admin`：

| 谁 | 打什么 | 结果 |
|---|---|---|
| renovate（write，不在白名单） | `v0.0.1-test` | ❌ `422 user not allowed to create protected tag` |
| forgejo-admin（白名单内） | `v1.0.0` | ✅ 201 |
| renovate | **`build-1`** | ✅ **201 直接放行** |

第三行是重点：**保护只覆盖匹配的模式**。发版 workflow 如果对任意 tag 触发、
而保护只配了 `v*`，那道门是空的。
